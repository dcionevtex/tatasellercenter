...
export function registerCatalogTools(server: McpServer) {
  ...
  server.registerTool(
    "vtex_update_shipping_policy",
    {
      ...
      description:
        "PUT /api/logistics/pvt/shipping-policies/{id} — updates a policy. Omitted fields keep their current value: the endpoint itself is a full replace that resets anything left out, so this tool reads the policy and merges your changes over it before sending. Returns the policy as read back afterwards.",
      inputSchema: z.object({
        id: z.string(),
        updates: z
          .object({
