...
export function registerCatalogTools(server: McpServer) {
  ...
  server.registerTool(
    "vtex_update_dock",
    {
      ...
      description:
        "POST /api/logistics/pvt/configuration/docks with the id in the body — the update shares the create endpoint; posting to /docks/{dockId} is rejected by VTEX. Omitted fields keep their current value: the endpoint replaces the whole record, so this tool reads the dock and merges your changes before sending. Use `freightTableIds` to attach shipping policies and `salesChannels` to attach trade policies: both links live on the DOCK, not on the policy, and a policy with no dock never appears in shipping simulation. Pass the full list you want, existing ids included — e.g. [\"1\",\"2\"] to keep policy 1 and add policy 2.",
      inputSchema: z.object({
        dockId: z.string(),
        name: z.string().optional(),
        warehouseIds: z.array(z.string()).optional(),
        ...
        salesChannels: z
          ...
          .optional()
          .describe(
            "Trade policy (sales channel) ids this dock sells on, e.g. [\"1\"]. Replaces the current list. See vtex_list_trade_policies."
          ),
        priority: z.number().int().optional(),
        isActive: z.boolean().optional(),
      }),
